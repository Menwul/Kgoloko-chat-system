...
from flask import Flask, render_template, request, redirect, request, url_for, session, jsonify, flash, send_from_directory
import sqlite3
...
def chat():
    ...
    # Filter rooms based on user role (admin sees all)
    available_rooms = []
...
def send_message():
    try:
        ...
        # Check if user role is allowed (admin can access all rooms)
        ...
        if reply_to:
            try:
                ...
                # Verify the replied message exists in the same room
                c.execute("SELECT id FROM messages WHERE id = ? AND room = ?", (reply_to, room))
                if not c.fetchone():
                    reply_to = None  # Invalid reply reference
        ...
        c.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (room, session['username'], message_content, message_type, file_path, timestamp, reply_to))
        
        message_id = c.lastrowid
        
        # If this is an edit of an existing message, handle it
        edit_id = request.form.get('edit_id')
        if edit_id:
            try:
                edit_id = int(edit_id)
                # Verify the user owns the message they're trying to edit
                c.execute("""
                    UPDATE messages 
                    SET is_edited = TRUE, edited_at = ?
                    WHERE id = ? AND username = ? AND room = ?
                """, (timestamp, edit_id, session['username'], room))
                
                if c.rowcount == 0:
                    print("Warning: User attempted to edit a message they don't own or that doesn't exist")
            except (ValueError, TypeError):
                pass  # Invalid edit ID, ignore
...
def get_messages(room):
    ...
    # Allow admin to access all rooms
    if session['role'] != 'admin' and session['role'] not in allowed_roles:
        conn.close()
        return jsonify({'error': 'Access denied to this room'}), 403
...
def edit_message():
    try:
        ...
        # Verify the user owns the message and it's a text message
...
def delete_message():
    try:
        ...
        # Delete the message without ownership or room checks
